    ...
    const downloadReport = async (e: React.FormEvent) => {
        ...
        fetch("/RMS-FORMv2.xlsx")
            ...
            .then(workbook => {
                ...
                worksheet.getCell('U107').value = data.msword == "A" ? "Advanced/ขั้นสูง" : data.msword == "F" ? "Intermediate/ระดับกลาง" : data.msword == "P" ? "Basic/พื้นฐาน" : "Advanced and able to training to other/ขั้นสูงและสอนคนอื่นได้";
                worksheet.getCell('A108').value = data.otherLanguageTest
                worksheet.getCell('F108').value = data.ieltsScore
                worksheet.getCell('U108').value = data.msexcel == "A" ? "Advanced/ขั้นสูง" : data.msword == "F" ? "Intermediate/ระดับกลาง" : data.msword == "P" ? "Basic/พื้นฐาน" : "Advanced and able to training to other/ขั้นสูงและสอนคนอื่นได้";
