    ...
    const downloadReport = async (e: React.FormEvent) => {
        ...
        fetch("/RMS-FORMv2.xlsx")
            ...
            .then(workbook => {
                ...
                worksheet.getCell('R15').value = data.weight
                ...
                for (let i = 0; i < data.rmsInternship.length; i++) {
                    worksheet.getCell('A' + (64 + i)).value = moment(data.rmsInternship[i].internshipExpFrom).format("MMM yyyy") + " - " + moment(data.rmsInternship[i].internshipExpTo).format("MMM yyyy")
                    worksheet.getCell('E' + (64 + i)).value = data.rmsInternship[i].internshipCompany
                    worksheet.getCell('O' + (64 + i)).value = data.rmsInternship[i].internshipPosition
                    worksheet.getCell('T' + (64 + i)).value = data.rmsInternship[i].internshipTypeofBusiness
                }

                if (data.newGraduate === "Y") {
                    worksheet.mergeCells('A70:V70'); // Merge row 70 จากคอลัมน์ A ถึง V
                    worksheet.mergeCells('A71:V71'); // Merge row 71 จากคอลัมน์ A ถึง V
                    worksheet.getCell('B71').value = "นักศึกษาจบใหม่ / New Graduate";
                }
